...
class OksCost(object):
    ...
    def __init__(self, num_keypoints=17, weight=1.0):
        ...
        elif num_keypoints == 22: #TODO CHANGE THESE VALUES
            self.sigmas = np.array([
                .26, .26,
                1.07, 1.07,
                .45, .45,
                ...
                .45, .45,
                .45, .45,
                .45, .45], dtype=np.float32) / 10.0
        elif num_keypoints == 19: 
            self.sigmas = np.array([
                 .26, .26, 1.07,  .79,  .72,  .62,  1.07, .87, .89, 
                 .79, .72, .62, 1.07, .87, .89, .25, .25, .35, .35,
    ...
    def __call__(self, kpt_pred, gt_keypoints, valid_kpt_flag, gt_areas):
        ...
        for i in range(len(gt_keypoints)):
            ...
            if num_vis_kpt > 0:
                ...
                oks = torch.zeros((kpt_pred.shape[0], 1)).cuda()
