...
def vis_results(visualizer, img, result, target):
    inv_normalize = T.Normalize(
        mean=[-0.485 / 0.229, -0.456 / 0.224, -0.406 / 0.255],
        std=[1 / 0.229, 1 / 0.224, 1 / 0.255]
    )

    imgs = [inv_normalize(img).cpu(), inv_normalize(img).cpu()]
    img_ids = [target['image_id'].item(), target['image_id'].item()]
    ...
    for ax, img, img_id in zip(axarr, imgs, img_ids):
        ...
        ax.text(
            0, 0, f'IMG_ID={img_id}',
            fontsize=5, bbox=dict(facecolor='white', alpha=0.5))
    ...
    for box_id in range(len(keep)):
        ...
        kpts = result['kpts'][box_id]
        kpts = clip_boxes_to_image(kpts, target['size'])
        kpts = kpts.reshape(-1, 2)
        for k in kpts:
            axarr[0].add_patch(plt.Circle((k[0], k[1]), 3, color='red', fill=True))
    ...
    i = 1
    cmap = plt.cm.get_cmap('hsv', len(target['track_ids']))

    for j, track_id in enumerate(target['track_ids']):
        x1, y1, x2, y2 = target['boxes'][j]
        axarr[i].text(
        ...
        # kpts = frame_target['keypoints'][j]
        # for k in kpts:
        #     axarr[i].add_patch(plt.Circle((k[0], k[1]), 3, color='yellow', fill=True))
        if 'masks' in target:
...
def build_visualizers(args: dict, train_loss_names: list):
    ...
    if args.EVAL_MODE or args.VISDOM.no_vis or not args.VISDOM.vis_server:
        return visualizers

    env_name = str(args.OUTPUT).split('/')[-1]

    vis_kwargs = {
        'env': env_name,
        'resume': args.RESUME.resume and args.RESUME.resume_vis,
        'port': args.VISDOM.vis_port,
        'server': args.VISDOM.vis_server}
