...
from .box_ops import joint_relative_to_absolute, joint_absolute_to_relative
...
def plot_sequence(tracks, data_loader, output_dir, write_images, generate_attention_maps):
    ...
    img = cv2.imread("/mnt/beegfs/work/ToyotaHPE/datasets/Panoptic/training/160906_band3/hdImgs/00_06/00_06_00000002.jpg")
    for track_id, track_data in tracks.items():
        for j, track in enumerate(track_data):
            bbox = track_data[track]['bbox']
            kpt = track_data[track]['kpt']
            new_shape = (kpt.shape[-1]//2, 2)
            kpt = np.reshape(kpt, new_shape)
            kpt = kpt[:, :2].transpose(1,0).reshape(-1)
            pose2d = [int(x) for x in kpt]
            bbox = [int(x) for x in bbox]
            keypoints = list(zip(pose2d[:19], pose2d[19:2*19])) # keypoints in format  (x_i, y_i)
            for keypoint in keypoints:
                cv2.circle(img, keypoint, 5, (0, j*2, 255), thickness=-1)
            cv2.circle(img, (bbox[0], bbox[1]), 5, (255, 0, j*3), thickness=-1)
            cv2.circle(img, (bbox[2], bbox[3]), 5, (j*3, 0, 255), thickness=-1)
            cv2.circle(img, ((bbox[0] + bbox[2])//2, (bbox[1] + bbox[3])//2), 5, (j*3, 255, 0), thickness=-1)
    cv2.imwrite("Test-Trajectory.jpg", (img))
    ...
    for frame_id, frame_data  in enumerate(tqdm.tqdm(data_loader)):
        ...
        for track_id, track_data in tracks.items():
            if frame_id in track_data.keys():
                ...
                if write_images == 'debug':
                    ...
                    cv2.putText(img, f"{track_id} - {track_data[frame_id]['obj_ind']} ({track_data[frame_id]['score']:.2f})",
        ...
        mmcv.imwrite(img, osp.join(output_dir, osp.basename(img_path)), auto_mkdir=True)
