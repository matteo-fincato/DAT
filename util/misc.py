...
if float(torchvision.__version__[2:4]) < 7.0:
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:  
        args.rank = int(os.environ["RANK"]) #+ ((int(os.environ['SLURM_PROCID']))*(int(os.environ['SLURM_GPUS_ON_NODE'])))
        os.environ["RANK"] = str(args.rank)
        args.world_size = int(os.environ['WORLD_SIZE'])
        args.gpu = int(os.environ['LOCAL_RANK'])
        args.slurm_id = int(os.environ['SLURM_PROCID'])
        args.slurm_gpus = int(os.environ['SLURM_GPUS_ON_NODE'])
        
        print(f'| (rank {args.rank}),  slurm id {args.slurm_id} on gpu {args.slurm_gpus} machine {socket.gethostname()}', flush=True)
    ...
    print(f'| distributed init (rank {args.rank}): {args.dist_url} on gpu {args.gpu} | {args.world_size}', flush=True)
    ...
    # print(f'| torch barrier (rank {args.rank}): {args.dist_url}', flush=True)
    # torch.distributed.barrier()
    print(f'| distributed init done (rank {args.rank}): {args.dist_url}', flush=True)
