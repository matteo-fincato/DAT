...
def crop(image, target, region, overflow_boxes=False):
    ...
    fields = ["labels", "area", "iscrowd", "ignore", "track_ids", "keypoints"]
    ...
    if "keypoints" in target:
        ...
        max_size = torch.as_tensor([w, h], dtype=torch.float32)
        cropped_kpts = kpts - torch.as_tensor([j, i, 0.0])
        
        for i, kpts in enumerate(cropped_kpts):
            for j, kpt in enumerate(kpts):
                ...
                if y > h:
                    y = h ; vis = 0.0
                cropped_kpts[i,j] = torch.tensor([x, y, vis], dtype=kpt.dtype)
        target["keypoints"] = cropped_kpts
...
def hflip(image, target):
    ...
    if "keypoints" in target:
        kpt = target["keypoints"]
        kpt = kpt \
            * torch.as_tensor([-1, 1, 1]) \
            + torch.as_tensor([width, 0, 0])
        
        #swap keypoints
        if kpt.shape[1]==19:
            # 0: Neck, 1: Nose,  2: BodyCenter(center of hips),
            # 3: lShoulder,  4: lElbow,  5: lWrist,  6: lHip,  7: lKnee  8: lAnkle,
            # 9: rShoulder, 10: rElbow, 11: rWrist, 12: rHip, 13: rKnee, 14: rAnkle,
            # 15: rEye, 16: lEye,
            # 17: rEar, 18: lEar
            permutation_index = [0,1,2, 9,10,11,12,13,14, 3,4,5,6,7,8, 16,15,18,17]
            kpt=kpt[:,permutation_index,:]
            
        target["keypoints"] = kpt
...
def resize(image, target, size, max_size=None):
    ...
    if "keypoints" in target:
        kpts = target["keypoints"]
        scaled_kpts = kpts \
            * torch.as_tensor([ratio_width, ratio_height, 1.0])
        target["keypoints"] = scaled_kpts
...
class Normalize:
    ...
    def __call__(self, image, target=None):
        ...
        if "keypoints" in target:
            kpts = target["keypoints"]
            # boxes = box_xyxy_to_cxcywh(boxes)
            kpts = kpts / torch.tensor([w, h, 1.0], dtype=torch.float32)
